...
RC IndexFileHeader::init(const IndexMeta &index_meta)
{
...
RC BplusTreeHandler::create(
    LogHandler &log_handler, BufferPoolManager &bpm, const char *file_name, const IndexMeta &index_meta)
{
  ...
  rc = this->create(log_handler, *bp, index_meta);
  if (OB_FAIL(rc)) {
...
RC BplusTreeHandler::create(LogHandler &log_handler, DiskBufferPool &buffer_pool, const IndexMeta &index)
{
