...
RC BplusTreeIndex::create(Table *table, const char *file_name, const IndexMeta &index_meta)
{
  ...
  if (RC::SUCCESS != rc) {
    LOG_WARN("Failed to create index_handler, file_name:%s, index:%s, field:%s, rc:%s",
        file_name, index_meta.name(), index_meta.to_string().c_str(), strrc(rc));
    return rc;
  }
...
{
  if (inited_) {
    LOG_INFO("Begin to close index, index:%s, field:%s", index_meta_.name(), index_meta_.to_string().c_str());
...
{
  char *entry = index_meta_.make_entry_from_record(record);
  RC    rc    = index_handler_.delete_entry(entry, rid);
  delete[] entry;
  return rc;
}
