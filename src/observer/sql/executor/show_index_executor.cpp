...
RC ShowIndexExecutor::execute(SQLStageEvent *sql_event)
{
  ...
  if (table != nullptr) {
    ...
    tuple_schema.append_cell(TupleCellSpec("", "Index_name", "Index_name"));
    ...
    for (int i = 0; i < table_meta.index_num(); i++) {
      ...
      for (size_t j = 0; j < index_meta->fields().size(); j++) {
        auto index_name = index_meta->name();
        auto field_name = index_meta->fields()[j].name();
        auto id         = std::to_string(j + 1);
        auto unique     = index_meta->unique() ? "0" : "1";
        oper->append({table_name, index_name, field_name, id, unique});
      }
    }
